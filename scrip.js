...
const img = new Image(); // สร้างออบเจ็กต์ภาพ
	img.src = "./gettyimages-180680638-676f621f720bc.avif"; // กำหนด URL ของภาพ
...
let direction = 1;
	...
	function animate() {
		...
		const deltaTime = (currentTime - lastTime) / 1000; // คำนวณเวลาที่ผ่านไปตั้งแต่เฟรมก่อนหน้า
		...
		ctx.drawImage(img,-1000,-400,2000, 800);
		ctx.restore(); // คืนสถานะ context กลับไปยังสถานะที่บันทึกไว้ก่อนหน้านี้
		
	

		angle += (Math.PI / -2) * deltaTime; // เพิ่มมุมเพื่อให้เกิดการหมุนในเฟรมถัดไป
